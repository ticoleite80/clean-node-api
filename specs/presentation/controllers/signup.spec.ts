...
import { AddAccount, AddAccountModel, AccountModel, EmailValidator } from '../../../src/presentation/controllers/signup/signup-protocols'
...
describe('Signup Controller', () => {
  ...
  test('Shoud return 400 if no passowrd is provided', async () => {
    const { sut } = makeSut()
    const httpRequest = {
      body: {
        name: 'any_name',
  ...
  test('Shoud return 400 if no passowrd confirmation is provided', async () => {
    const { sut } = makeSut()
    const httpRequest = {
      body: {
        name: 'any_name',
        email: 'any_email@email.com',
        password: 'any_password',
        passwordConfirmation: 'invald_password'
      }
    }
    const httpResponse = await sut.handle(httpRequest)
    expect(httpResponse.statusCode).toBe(400)
  ...
  test('Shoud return 400 if passowrd confirmation fails', async () => {
    const { sut } = makeSut()
    const httpRequest = {
      body: {
        name: 'any_name',
